...
function raytrace() {
    ...
    for (x = 0; x < width; x++) {
        for (y = 0; y < height; y++) {
            ...
            //antialiasing
            for (subx = x ; subx < x + 1 ; subx+= 1/ antialiasing.n ) {
                for (suby = y ; suby < y + 1 ; suby += 1 / antialiasing.n) {
                    ...
                    //jitter
                    if (antialising.jittered) {
                        var step = 1 / antialising.n
                    ...
                    avg_pix += color / (antialiasing.n * antialiasing.n)
