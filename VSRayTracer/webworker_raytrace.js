    ...
    for (var y = 0; y < height; y++) {
        ...
        for (var xSample = 0; xSample < antialiasing.n; xSample++) {
            for (var ySample = 0; ySample < antialiasing.n; ySample++) {
                var x2 = x + xSample / antialiasing.n;
                var y2 = y + ySample / antialiasing.n;
                ...
                var RotXY = new THREE.Matrix4().multiplyMatrices(RotY, RotX);
    ...
    switch (obj.type) {
        case "sphere":
            ...
            if (disc > 0) {
                ...
                var candidates = [];
                if (t0 > 0.01)
                    candidates.push(p0.applyMatrix4(scale).add(obj.pos));
                if (t1 > 0.01)
